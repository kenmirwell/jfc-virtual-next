...
const Clouds = ({ title, delay, animate, color }) => {
    ...
    >
      {clouds.map((c) => (
        <div
          ...
          className={`${c.className} pointer-events-none clouds absolute flex items-center justify-center flex-wrap text-center`}
        >
          {c.svg}
        </div>
      ))}
      <div
        id='text'
        className='pointer-events-none absolute top-0 left-0 right-0 bottom-0 text-center flex items-center justify-center'
      >
        <h1
          id='mtitle'
          className='uppercase text-white text-[100px] leading-none font-[700]'
        >
              ...
              {w.split("").map((t, i) => (
                ...
                >
                  <span
                    ...
                    style={{
                      WebkitTextStroke: `5px ${color ? color : "#00bbdc"}`,
                    }}
                  >
                    {t}
                  </span>
